...
interface Props {
  /** 컨테이너 높이 제어용 클래스. 시트/페이지에 따라 조정. */
  className?: string;
  /** 시트 모드: 입력바 위치/하단 여백 처리 변경 */
  variant?: 'page' | 'sheet';
}

const ChatPanel = ({ className = '', variant = 'page' }: Props) => {
  const { user } = useAuth();
        ...
        <div className="mb-3 flex items-center justify-between rounded-xl border border-border bg-white px-3 py-2">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate">
              {profile.skinType ? `${profile.skinType} 피부에 맞춰 답해드려요` : '프로필을 입력하면 더 정확해져요'}
            </p>
          </div>
          {messages.length > 0 && (
            <button
              ...
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full hover:bg-neutral-100 text-muted-foreground"
              title="대화 초기화"
            >
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 mt-2">
            <div className="text-center space-y-2 py-4">
              ...
              <p className="text-sm font-bold text-foreground">무엇이 궁금하세요?</p>
