...
다음 JSON으로 반환:
{
  ...
  "score": 피부 건강 점수 (0-100)
      ...
      if (error || !data) {
        const fallbackType = determineSkinType(answers);
        setResult(fallbackType);
        return;
      }
      ...
      if (data.skinType) {
        ...
        setResult(determineSkinType(answers));
      }
    } catch {
      setResult(determineSkinType(answers));
    } finally {
  ...
  const determineSkinType = (a: Record<string, string>): DiagnosisResult => {
    let type = '복합성';
  ...
  return (
    <div className="min-h-screen bg-neutral-50 pb-24">
      ...
      <div className="h-1 bg-neutral-100">
        <div className={`h-full ${result ? 'bg-green-500' : 'bg-primary'} transition-all duration-300`} style={{ width: `${progress}%` }} />
      </div>
        ...
        {!result && !diagnosing && currentQ && (
          <div className="space-y-6">
            <div className="text-center">
              <p className="text-xl font-bold text-foreground whitespace-pre-line leading-relaxed">{currentQ.text}</p>
            </div>
            ...
            <div className="space-y-2.5">
              {currentQ.options.map(opt => (
                ...
                >
                  ...
                  <span className={`text-sm font-medium ${answers[currentQ.id] === opt.value ? 'text-primary' : 'text-foreground'}`}>{opt.label}</span>
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-3 pt-4">
              {step > 0 && (
                <button onClick={() => setStep(s => s - 1)} className="flex-1 rounded-xl border border-border py-3.5 text-sm font-semibold text-muted-foreground">
                  이전
                </button>
              )}
              <button
                ...
                className="flex-1 rounded-xl bg-primary py-3.5 text-sm font-bold text-primary-foreground disabled:opacity-40 flex items-center justify-center gap-1"
              >
        ...
        {result && (
          <div className="space-y-5 pb-8">
            ...
            <div className="rounded-2xl border border-border bg-white p-5 text-center shadow-card">
              ...
              <div className="w-full bg-neutral-100 rounded-full h-2.5 mt-4 mb-3">
                <div className={`h-2.5 rounded-full transition-all duration-700 ${scoreBg(result.score)}`} style={{ width: `${result.score}%` }} />
              </div>
            ...
            <div className="rounded-2xl border border-border bg-white p-4 shadow-card">
              <p className="text-sm font-bold text-foreground mb-3">내 피부 특징</p>
            ...
            <div className="rounded-2xl border border-green-200 bg-green-50 p-4 shadow-card">
              <p className="text-sm font-bold text-green-800 mb-3">맞춤 케어 추천</p>
              <div className="space-y-2">
            ...
            <div className="rounded-2xl border border-red-200 bg-red-50 p-4 shadow-card">
              <p className="text-sm font-bold text-red-800 mb-3">피해야 할 성분</p>
              <div className="flex flex-wrap gap-1.5">
                {result.avoidIngredients.map((a, i) => (
                  <span key={i} className="rounded-full border border-red-200 bg-white px-2.5 py-1 text-xs font-medium text-red-700">{a}</span>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={() => { setStep(0); setAnswers({}); setResult(null); setApplied(false); }}
                className="w-full rounded-xl border border-border py-3.5 text-sm font-semibold text-muted-foreground"
              >
                다시 진단하기
              </button>
            </div>
