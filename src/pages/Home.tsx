    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="gradient-primary px-5 pb-8 pt-12">
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          ...
          {profile.personalColor && profile.personalColor !== '모름' && (
            <span className="rounded-full bg-primary-foreground/20 px-2.5 py-0.5 text-xs font-medium text-primary-foreground">{profile.personalColor}</span>
          )}
        </div>
      </div>
      ...
      <div className="px-5">
        <div className="relative -mt-5">
          ...
          <Input placeholder="제품명 또는 브랜드 검색" value={query} onChange={e => setQuery(e.target.value)} className="rounded-xl border-border bg-card pl-10 shadow-sm" />
        </div>
